...
Ext.define('AM.plugins.ImageUpload', {
    ...
    referenceUrl : '/ext/uploadFile/download?uploadEntry=',
...
/**
   ...
   */
     ...
     */
    iframeCss: '/css/codemonkey-em/HtmlEditorImageUpload_iframe_styles.css',
    t: function (string) {
				...
				if (btn == 'yes') {
					Ext.Ajax.request({
						...
						success: function (response) {
							...
							if(result.success)
							{
								me.imgSrc = me.referenceUrl + result.data['id'];
...
Ext.define('Ext.ux.form.HtmlEditor.ImageDialog', {
        ...
        me.items = [{
            ...
            items: [{
                ...
                items: [{
                    ...
                    tpl: '<tpl for="."><table class="x-boundlist-item" style="width:50%"><tr><td style="vertical-align:top;width:12px"><tpl if="'+me.disableDelete+' == false"><a title="' + me.t('Delete Image') + '" href="#" img_fullname="{fullname}" class="x-htmleditor-imageupload-delete"></a></tpl></td><td><div class="x-htmleditor-imageupload-thumbcontainer"><img src="{thumbSrc}"/></div></td></tr><tr><td colspan="2" style="text-align:center;font-size:12px">{name}</td></tr></table></tpl>',
                    listConfig: {
        ...
        if (form.isValid()) {
            form.submit({
                ...
                success: function (fp, o) {
                    ...
                    var combo = me.down('[name=src]');
                    combo.needsRefresh = true;
                    combo.setRawValue(me.referenceUrl + o.result.data['id']);
                    me._setPreviewImage(me.referenceUrl + o.result.data['id'], true);
                },
		...
		Ext.Ajax.request({
			...
			success: function (response) {
				...
				if(result.success)
				{
					var combo = me.down('[name=src]');
                    combo.needsRefresh = true;
                    combo.setValue(me.referenceUrl + result.data['id']);
                    me._setPreviewImage(me.referenceUrl + result.data['id'], true);
    ...
    _setPreviewImage: function (src, resetImageSize) {
	    ...
	    var img = new Image();	
		img.src = src;
		img.previewComponent = this.previewComponent;
